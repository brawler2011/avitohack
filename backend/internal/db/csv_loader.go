...
func LoadCSVData(ctx context.Context, queries sqlc.Querier, usersPath, activitiesPath string) error {
	// 1. Load users
	...
	for i, row := range uRecords {
		...
		_, err := queries.UpsertUser(ctx, sqlc.UpsertUserParams{
			ID:           int32(id),
			Username:     row[1],
			FullName:     row[2],
			AvatarUrl:    row[3],
	...
	_ = queries.DeleteAllActivities(ctx)
	...
	for i, row := range aRecords {
		...
		_, err := queries.InsertActivity(ctx, sqlc.InsertActivityParams{
			ID:              int32(id),
			UserID:          int32(userID),
			Timestamp:       t,
			ActivityType:    row[3],
		...
		if err != nil {
			// Ignore or log error
			fmt.Printf("Warning: failed to insert activity row %d: %v\n", id, err)
		}
	}
